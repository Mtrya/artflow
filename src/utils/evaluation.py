...
def run_evaluation_uncond(
    ...
    with torch.no_grad():
        # --- 1. Generate Samples ---
        sample_z0 = torch.randn(args.num_eval_samples, 16, args.eval_resolution // 8, args.eval_resolution // 8, device=accelerator.device)
        ...
        try:
            vis_z0 = torch.randn(1, 16, args.eval_resolution // 8, args.eval_resolution // 8, device=accelerator.device)
            if args.algorithm == "fm-ot":
...
def run_evaluation_light(
    ...
    with torch.no_grad():
        ...
        dataset = load_dataset("kaupane/wikiart-captions", split="train[:args.num_eval_samples]")
        ...
        H = W = resolution
        sample_z0 = torch.randn(len(prompts), 16, H // 8, W // 8, device=accelerator.device)
        ...
        # Save locally
        save_path = os.path.join(args.output_dir, f"{args.run_name}/samples_step_{current_step:06d}.png")
        grid = make_image_grid(images, save_path=save_path, normalize=True, value_range=(-1, 1))
        print(f"Saved samples to {save_path}")
        ...
        if real_images:
            ...
            fid_score = calculate_fid(real_images_tensor, fake_images_norm, device=accelerator.device)
            print(f"Step {current_step} | FID: {fid_score:.4f}")
            metrics["fid"] = fid_score
            
        # CLIP Score
        clip_score = calculate_clip_score(fake_images_norm, prompts, device=accelerator.device)
...
def run_evaluation_heavy(
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(dataloader, desc="Generating")):
            ...
            if isinstance(batch["image"], list):
                ...
                imgs = [torchvision.transforms.functional.to_tensor(img) for img in batch["image"]]
    ...
    if real_images_all.shape[-2:] != fake_images_all.shape[-2:]:
        real_images_all = torch.nn.functional.interpolate(
            real_images_all, size=fake_images_all.shape[-2:], mode="bicubic", align_corners=False
        )
