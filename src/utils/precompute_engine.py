...
def _sample_resolution_bucket(
    ...
    if probs:
        ...
        if candidate_count == 0:
            candidate_count = len(sorted_bucket_ids)
        ...
        closeness = [1.0 / (distances[bucket_id] + 1e-6) for bucket_id in sorted_bucket_ids]
        total_closeness = sum(closeness)
        probabilities = [w / total_closeness for w in closeness]
        sampled_bucket_id = random.choices(sorted_bucket_ids, weights=probabilities, k=1)[0]
...
def _sample_local_mask(
    ...
    max_scale = float(np.clip(max_scale, min_scale if max_scale < min_scale else 1.0, 1.0))
...
def _apply_data_augmentations(
    ...
    Args:
        ...
        do_augmentation: Flag toggling whether random perturbations are applied.
    ...
    seed = augmentation_args.get("seed")
    rng = augmentation_args.get("rng")
    if isinstance(rng, random.Random):
        local_rng = rng
    else:
        local_rng = random.Random(seed if seed is not None else random.getrandbits(64))

    np_rng = augmentation_args.get("np_rng")
    if np_rng is None:
        np_rng = np.random.default_rng(local_rng.getrandbits(63))

    area_scale_range = augmentation_args.get("area_scale_range", (0.08, 0.2))
    aspect_ratio_range = augmentation_args.get("area_aspect_ratio_range", (0.75, 1.33))
    mask_blur_radius = float(augmentation_args.get("mask_blur_radius", 3.0))

    image_array = np.asarray(resized, dtype=np.float32) / 255.0

    gaussian_prob = float(augmentation_args.get("gaussian_prob", 0.0))
    if local_rng.random() < gaussian_prob:
        sigma_min, sigma_max = augmentation_args.get("gaussian_sigma_range", (0.5, 1.5))
    ...
    noise_prob = float(augmentation_args.get("noise_prob", 0.0))
    if local_rng.random() < noise_prob:
        std_min, std_max = augmentation_args.get("noise_std_range", (2.0, 6.0))
    ...
    color_prob = float(augmentation_args.get("color_jitter_prob", 0.0))
    if local_rng.random() < color_prob:
        jitter_params = augmentation_args.get("color_jitter_params", {
            "brightness": 0.05,
            "contrast": 0.05,
            "saturation": 0.05,
            "hue": 0.02,
...
class PrecomputeEngine:
    ...
    def __init__(
        ...
        do_caption_scheduling: bool = False,
        ...
        self.vae = vae
        self.do_caption_scheduling = do_caption_scheduling
        self.caption_scheduling_args = caption_scheduling_args or {}
        self.do_data_augmentation = do_data_augmentation
        self.data_augmentation_args = dict(data_augmentation_args or {})
        ...
        self.resolution_probs = None
        if "resolution_probs" in self.data_augmentation_args:
            probs = self.data_augmentation_args.pop("resolution_probs")
            if probs is not None:
                self.resolution_probs = list(probs)

        self.text_cache: Dict[Tuple[str, bool], Optional[Tuple[torch.Tensor, torch.Tensor, Any]]] = {}

        rng = self.data_augmentation_args.get("rng")
        if isinstance(rng, random.Random):
            self._rng = rng
        else:
            seed = self.data_augmentation_args.get("seed")
            self._rng = random.Random(seed) if seed is not None else random.Random()
        self.data_augmentation_args["rng"] = self._rng
    ...
    def run(self, dataset: Dataset, map_batch_size: int = 8) -> Dataset:
        """Apply pre-computation to a dataset using the persistent cache."""

        stage = float(self.caption_scheduling_args.get("stage", 0.0)) if self.do_caption_scheduling else None
        min_prob = float(self.caption_scheduling_args.get("min_prob", 0.15))
        max_prob = float(self.caption_scheduling_args.get("max_prob", 0.8))

        resolution_buckets = self.resolution_buckets
        size_to_bucket = self.size_to_bucket
        caption_fields = self.caption_fields
        text_cache = self.text_cache

        augmentation_args = {**self.data_augmentation_args, "rng": self._rng}
        ...
        def _process_batch(batch: Dict[str, List[Any]]) -> Dict[str, List[Any]]:
            ...
            for idx in range(batch_size):
                ...
                bucket_id = size_to_bucket[tuple(resolution)]
                ...
                sampled_caption = (
                    _sample_caption(caption_candidates, stage or 0.0, min_prob, max_prob)
                    if self.do_caption_scheduling
                    else caption_candidates[0]
                )
            ...
            if missing_captions:
                ...
                for idx, cache_key in enumerate(missing_keys):
                    ...
                    text_cache[cache_key] = (embeddings[idx], masks[idx], pooled_value)

            text_embeddings = [text_cache[(caption, self.pooling)][0] for caption in captions]
            attention_masks = [text_cache[(caption, self.pooling)][1] for caption in captions]
            pooled_embeddings = [text_cache[(caption, self.pooling)][2] for caption in captions]
...
def precompute(
    ...
    pooling: bool,
    vae: Any,
    resolution_buckets: Dict,
    do_caption_scheduling: bool,
    caption_scheduling_args: Dict,
    ...
    return engine.run(dataset, map_batch_size=map_batch_size)
