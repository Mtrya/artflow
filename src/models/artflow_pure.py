"""
Pure Double-Stream DiT model for ArtFlow (Stage 1: Architecture Ablation).
Conditioning: MLP(t) only.
"""
...
class ArtFlowPure(nn.Module):
    """
    Double-stream DiT with timestep-only conditioning in AdaLN.
    Text is processed via double-stream attention but not fused into AdaLN.
    """
    def __init__(
        self,
        patch_size: int = 2,
...
if __name__ == "__main__":
    ...
    model = ArtFlowPure(hidden_size=320, depth=4, num_heads=4, txt_in_features=2048)
