...
GENRE_MAP = {
    ...
    8: "sketch and study", 9: "still life", 10: "painting of unknown genre"
}
...
def save_checkpoint(processed_ds: Dataset, next_idx: int) -> None:
    ...
    print(f"Checkpoint saved: {len(processed_ds)} examples processed, next start: {next_idx}")
...
def load_checkpoint() -> Tuple[Optional[Dataset], int]:
    ...
    if os.path.exists(checkpoint_path):
        ...
        print(f"Resuming from checkpoint: {checkpoint_info['total_processed']} examples already processed")
...
def add_captions(batch):
    ...
    if batch.get(f"{captioners[0]["name"]}") is None: # first round, no existing captions, need to initialize the caption keys
        ...
        for i in range(len(images)):
            ...
            existing = {f"{c["name"]}": batch.get(f"{c["name"]}")[i] for c in captioners}
...
def curate_dataset(
    ...
    if model.lower() == "qwen":
        ...
        raise ValueError(f"Model set other than 'qwen' and 'mistral' not defined.")
