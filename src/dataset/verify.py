...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Path to VAE model for image reconstruction",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output/verification_samples",
    ...
    if "latents" in dataset.column_names:
        ...
        if latents_subset.numel() > 0:
            if latents_subset.dim() > 1:
                ...
                print(
                    f"Latent stats calculated on flattened subset of {subset_size} samples."
                )
    ...
    if args.num_samples > 0:
        ...
        for i, idx in enumerate(indices):
            ...
            if vae and "latents" in example:
                latent = (
                    torch.tensor(example["latents"]).to(args.device).to(torch.bfloat16)
                )
                # Add batch dim
