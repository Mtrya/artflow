...
def test_timestep_embeddings():
    ...
    print_test_result(shape_correct, f"Output shape: {output.shape} (expected: {expected_shape})")
...
def test_apply_rotary_emb():
    ...
    print_test_result(shape_correct, f"Output shape: {output.shape} (expected: {x.shape})")
    
    # Check dtype matches input
    dtype_correct = output.dtype == x.dtype
    print_test_result(dtype_correct, f"Output dtype: {output.dtype} (expected: {x.dtype})")
...
def test_msrope():
    ...
    print_test_result(img_shape_correct, f"Image freqs shape: {img_freqs.shape} (expected: {expected_img_shape})")
    
    # Check text frequencies shape
    expected_txt_shape = (77, 64)
    txt_shape_correct = txt_freqs.shape == expected_txt_shape
    print_test_result(txt_shape_correct, f"Text freqs shape: {txt_freqs.shape} (expected: {expected_txt_shape})")
    
    # Check dtype is complex
    img_complex = img_freqs.dtype == torch.complex64 or img_freqs.dtype == torch.complex128
    txt_complex = txt_freqs.dtype == torch.complex64 or txt_freqs.dtype == torch.complex128
    print_test_result(img_complex and txt_complex, f"Frequencies are complex (img: {img_freqs.dtype}, txt: {txt_freqs.dtype})")
    
    # Test caching
    img_freqs2, txt_freqs2 = rope(img_hw, txt_seq_len, device)
    cache_works = torch.allclose(img_freqs, img_freqs2) and torch.allclose(txt_freqs, txt_freqs2)
    print_test_result(cache_works, "Caching works correctly")
    
    return img_shape_correct and txt_shape_correct and img_complex and txt_complex and cache_works
...
def test_single_stream_dit_block():
    ...
    model = SingleStreamDiTBlock(dim=dim, num_heads=num_heads, c_dim=c_dim, rope_axes_dim=[64, 64])
    ...
    print_test_result(img_shape_correct, f"Image output shape: {out_img.shape} (expected: {img_tokens.shape})")
    print_test_result(txt_shape_correct, f"Text output shape: {out_txt.shape} (expected: {txt_tokens.shape})")
    ...
    out_img_masked, out_txt_masked = model(img_tokens, txt_tokens, c, img_hw, txt_seq_len, txt_attention_mask=txt_mask)
    
    mask_affects = not torch.allclose(out_txt, out_txt_masked)
    print_test_result(mask_affects, "Attention mask affects text output")
    
    return img_shape_correct and txt_shape_correct and img_modified and txt_modified and mask_affects
...
def test_double_stream_attention():
    ...
    txt_mask = torch.ones(batch_size, txt_seq_len)
    txt_mask[:, 50:] = 0
    out_img_masked, out_txt_masked = model(img_tokens, txt_tokens, img_hw, txt_seq_len, txt_attention_mask=txt_mask)
...
def test_double_stream_dit_block():
    ...
    model = DoubleStreamDiTBlock(dim=dim, num_heads=num_heads, c_dim=c_dim, rope_axes_dim=[64, 64])
...
def test_unconditional_dit_block():
    ...
    model = UnconditionalDiTBlock(dim=dim, num_heads=num_heads, c_dim=c_dim, rope_axes_dim=[64, 64])
    ...
    print_test_result(shape_correct, f"Output shape: {out.shape} (expected: {img_tokens.shape})")
